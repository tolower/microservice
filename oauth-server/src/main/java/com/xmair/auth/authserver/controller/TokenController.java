...
public class TokenController {
    ...
    /**
     * 客户端向认证服务器申请令牌(AccessToken) [验证client_id、client_secret、 code的正确性或更新令牌 refresh_token]
     * @param request
     * @param response
     * @return
    ...
    public void access_token(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //todo 验证redirecturl格式是否合法 、验证appkey是否正确 、AppSecret是否正确
            ...
            MemCacheUtil.set(refreshToken,accessToken);
            //将accesstoken与userid关联起来
            MemCacheUtil.set(accessToken,MemCacheUtil.get(MemCacheUtil.get(authzCode)));
            logger.info("accessToken : "+accessToken +"  refreshToken: "+refreshToken+"userid"+MemCacheUtil.get(MemCacheUtil.get(authzCode)));
            //清除授权码 确保一个code只能使用一次
            MemCacheUtil.remove(authzCode);
            //构建oauth2授权返回信息
            OAuthResponse oauthResponse = OAuthASResponse
    ...
    public void refresh_token(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            * */
            Object cache_refreshToken=MemCacheUtil.get(oauthRequest.getRefreshToken());
            //access_token已超时
            if (cache_refreshToken == null) {
                ...
                MemCacheUtil.set(refresh_Token,access_Token);
            ...
            //access_token未超时
            MemCacheUtil.set(oauthRequest.getRefreshToken(),cache_refreshToken.toString());
            //构建oauth2授权返回信息
            OAuthResponse oauthResponse = OAuthASResponse
                    .tokenResponse(HttpServletResponse.SC_OK)
